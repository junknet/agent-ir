 ...
 * `src/protocols.ts` 字节相同的 `INGRESS_PATHS`。两处各自维护，新增或改名一个协议时
 * 漏改哪一张都不会有任何东西报错：注册表用旧函数、路由指向不存在的端点，都只在运行时冒出来。
 *
 * 现在协议 → codec 的注册表只有 `INGRESS_CODECS` 一份，本文件的分发直接查它。
 */
import { INGRESS_CODECS } from "../protocols.ts";
...
export function readClientRequestForProtocol(protocol: IRProtocol, raw: unknown, traceId: string): ClientRequestReadResult {
  return INGRESS_CODECS[protocol].readClientRequest(raw, traceId);
}
